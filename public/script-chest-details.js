// ===== GLOBAL VARIABLES =====
let scene, camera, renderer, chestModel;
...
// لون هايلايت موحد لكل أجزاء الصدر
const UNIFIED_HIGHLIGHT_COLOR = 0x2563eb;

// خريطة أجزاء الصدر حسب الفهرس
const chestPartsByIndex = [
    { name: 'neck', ar: 'الرقبة', en: 'Neck', color: '#ef4444', icon: 'custom-icon neck' },
    { name: 'chest_up', ar: 'أعلى الصدر', en: 'Upper Chest', color: '#ef4444', icon: 'fas fa-lungs' },
    { name: 'sternum', ar: 'عظم القص', en: 'Sternum', color: '#ef4444', icon: 'fas fa-bone' },
    { name: 'breasts', ar: 'الثديان', en: 'Breasts', color: '#ef4444', icon: 'fas fa-heart' }
];


// ===== INITIALIZATION =====
async function init() {
    console.log("🚀 بدء تحميل تفاصيل الصدر...");
...
viewer.innerHTML = '';
    ...
    setupLighting();
    await loadChestModel();
    setupEventListeners();
    animate();
    
    console.log("✅ تهيئة تفاصيل الصدر اكتملت!");
}
...
async function loadChestModel() {
    console.log("🫁 جاري تحميل نموذج الصدر التفصيلي...");
    
    try {
        const loader = new THREE.GLTFLoader();
        loader.load('chest.gltf', (gltf) => {
            console.log("✅ تم تحميل نموذج الصدر!");
            
            chestModel = gltf.scene;
            clickableObjects = [];
            
            // معالجة نموذج الصدر
            processChestModel(chestModel);
            ...
            console.log(`🫁 تم العثور على ${clickableObjects.length} جزء من الصدر`);
            
            showNotification('تم تحميل تفاصيل الصدر بنجاح', 'success');
            
        }, undefined, (error) => {
            console.error('❌ خطأ في تحميل نموذج الصدر:', error);
            showNotification('لا يمكن تحميل تفاصيل الصدر', 'error');
            setTimeout(createBackupChestModel, 1000);
        });
...
// ===== PROCESS HEAD MODEL =====
function processChestModel(chestModel) {
    let partIndex = 0;
    ...
    chestModel.traverse((child) => {
        if (child.isMesh) {
            ...
            const partInfo = chestPartsByIndex[partIndex] || 
                           { name: 'Chest', ar: 'الصدر', en: 'Chest', color: '#ef4444', icon: 'fas fa-lungs' };
    ...
    console.log(`📊 تم معالجة ${clickableObjects.length} جزء من الصدر`);
}
...
function onModelClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        console.log("🎯 تم النقر على جزء من الصدر!");
        ...
        showNotification('الرجاء النقر مباشرة على جزء من الصدر', 'info');
    }
}
...
window.selectChestPart = function(partKey) {
...
function centerAndScaleModel() {
    ...
    box.setFromObject(chestModel);
    const center = box.getCenter(new THREE.Vector3());
    chestModel.position.x = -center.x;
    chestModel.position.y = -center.y;
    chestModel.position.z = -center.z;
...
function createBackupChestModel() {
    ...
    showNotification('تم تحميل تفاصيل الصدر الاحتياطية', 'info');
}

// ===== INITIALIZE =====
window.addEventListener('DOMContentLoaded', () => {
    console.log("🫁 بدء تشغيل تفاصيل الصدر...");
