...
// لون هايلايت موحد لكل أجزاء الرجل اليمنى
...
// خريطة أجزاء الرجل اليمنى (حسب الاسم أو الفهرس)
const partMapByName = {
    'thigh_R': { name: 'thigh_R', ar: 'فخذ', en: 'Thigh', icon: 'custom-icon bicep_alt' },
    'knee_R': { name: 'knee_R', ar: 'ركبة', en: 'Knee', icon: 'custom-icon knee' },
    'shin_R': { name: 'shin_R', ar: 'ساق', en: 'Shin', icon: 'custom-icon shin' },
    'ankle_R': { name: 'ankle_R', ar: 'كاحل', en: 'Ankle', icon: 'custom-icon ankle' },
    'foot_R': { name: 'foot_R', ar: 'قدم', en: 'Foot', icon: 'custom-icon foot_sole' },
};

const partsByIndex = [
    { name: 'thigh_R', ar: 'فخذ', en: 'Thigh', icon: 'custom-icon bicep_alt' },
    { name: 'knee_R', ar: 'ركبة', en: 'Knee', icon: 'custom-icon knee' },
    { name: 'shin_R', ar: 'ساق', en: 'Shin', icon: 'custom-icon shin' },
    { name: 'ankle_R', ar: 'كاحل', en: 'Ankle', icon: 'custom-icon ankle' },
    { name: 'foot_R', ar: 'قدم', en: 'Foot', icon: 'custom-icon foot_sole' },
];


// ===== INITIALIZATION =====
async function init() {
    console.log("🚀 بدء تحميل تفاصيل الرجل اليمنى...");
...
viewer.innerHTML = '';
    ...
    console.log("✅ تهيئة تفاصيل الرجل اليمنى اكتملت!");
}
...
async function loadBellyModel() {
    console.log("🫁 جاري تحميل نموذج الرجل اليمنى التفصيلي...");
    
    try {
        const loader = new THREE.GLTFLoader();
        loader.load('leg_R.gltf', (gltf) => {
            console.log("✅ تم تحميل نموذج الرجل اليمنى!");
            
            armModel = gltf.scene;
            clickableObjects = [];
            
            // معالجة نموذج الرجل اليمنى
            ...
            console.log(`🫁 تم العثور على ${clickableObjects.length} جزء من الرجل اليمنى`);
            
            showNotification('تم تحميل تفاصيل الرجل اليمنى بنجاح', 'success');
            
        }, undefined, (error) => {
            console.error('❌ خطأ في تحميل نموذج الرجل اليمنى:', error);
            showNotification('لا يمكن تحميل تفاصيل الرجل اليمنى', 'error');
...
function processBellyModel(armModel) {
    ...
    console.log(`📊 تم معالجة ${clickableObjects.length} جزء من الرجل اليمنى`);
}
...
function onModelClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        console.log("🎯 تم النقر على جزء من الرجل اليمنى!");
        ...
        showNotification('الرجاء النقر مباشرة على جزء من الرجل اليمنى', 'info');
    }
}
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.body-part-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            // إيجاد الميش المطابق لجزء الرجل اليمنى
...
function createBackupBellyModel() {
    ...
    showNotification('تم تحميل تفاصيل الرجل اليمنى الاحتياطية', 'info');
}

// ===== INITIALIZE =====
window.addEventListener('DOMContentLoaded', () => {
    console.log("🫁 بدء تشغيل تفاصيل الرجل اليمنى...");
...
function translateSelected_leg_RTranslations(meshName) {
  return leg_RTranslations[meshName] || meshName;
}
