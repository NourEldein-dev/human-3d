...
// لون هايلايت موحد لكل أجزاء الرأس
const UNIFIED_HIGHLIGHT_COLOR = 0x2563eb;

// خريطة أجزاء الرأس حسب الفهرس
const headPartsByIndex = [
    { name: 'Scalp', ar: 'فروة الرأس', en: 'Scalp', color: '#fbbf24', icon: 'custom-icon head' },
    { name: 'Nose', ar: 'الأنف', en: 'Nose', color: '#fbbf24', icon: 'custom-icon nose' },
    { name: 'Eyes', ar: 'العينان', en: 'Eyes', color: '#fbbf24', icon: 'fas fa-eye' },
    { name: 'Ears', ar: 'الأذنان', en: 'Ears', color: '#fbbf24', icon: 'fas fa-ear' },
    { name: 'Jaw', ar: 'الفك', en: 'Jaw', color: '#fbbf24', icon: 'fas fa-teeth' }
];

// ===== INITIALIZATION =====
async function init() {
    console.log("🚀 بدء تحميل تفاصيل الرأس...");
...
viewer.innerHTML = '';
    ...
    console.log("✅ تهيئة تفاصيل الرأس اكتملت!");
}
...
async function loadHeadModel() {
    console.log("🧠 جاري تحميل نموذج الرأس التفصيلي...");
    
    try {
        const loader = new THREE.GLTFLoader();
        loader.load('head.gltf', (gltf) => {
            console.log("✅ تم تحميل نموذج الرأس!");
            
            headModel = gltf.scene;
            clickableObjects = [];
            
            // معالجة نموذج الرأس
            processHeadModel(headModel);
            ...
            console.log(`🧠 تم العثور على ${clickableObjects.length} جزء من الرأس`);
            
            showNotification('تم تحميل تفاصيل الرأس بنجاح', 'success');
            
        }, undefined, (error) => {
            console.error('❌ خطأ في تحميل نموذج الرأس:', error);
            showNotification('لا يمكن تحميل تفاصيل الرأس', 'error');
            setTimeout(createBackupHeadModel, 1000);
        });
...
// ===== PROCESS HEAD MODEL =====
function processHeadModel(headModel) {
    let partIndex = 0;
    ...
    headModel.traverse((child) => {
        if (child.isMesh) {
            ...
            // التعرف على جزء الرأس حسب الفهرس
            const partInfo = headPartsByIndex[partIndex] || 
                           { name: 'Face', ar: 'الوجه', en: 'Face', color: '#fbbf24', icon: 'fas fa-user' };
    ...
    console.log(`📊 تم معالجة ${clickableObjects.length} جزء من الرأس`);
}
...
function onModelClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        console.log("🎯 تم النقر على جزء من الرأس!");
        ...
        showNotification('الرجاء النقر مباشرة على جزء من الرأس', 'info');
    }
}
...
// ===== SELECT PART FROM UI (ICONS/LIST) =====
function selectPartByName(partName) {
    if (!partName) return;

    if (!clickableObjects || clickableObjects.length === 0) {
        console.warn("⚠️ الأجزاء القابلة للنقر غير جاهزة بعد");
        return;
    }

    const targetMesh = clickableObjects.find(obj => obj?.userData?.bodyPart === partName);

    if (!targetMesh) {
        console.warn("⚠️ لم يتم العثور على Mesh مطابق للجزء:", partName);
        return;
    }

    // إزالة الهايلايت السابق ثم تمييز الجزء المطلوب
    resetHighlight();
    highlightMesh(targetMesh);

    // (اختياري) إشعار بسيط عند الضغط على الأيقونات
    if (targetMesh?.userData?.arabicName) {
        showNotification(`تم تحديد ${targetMesh.userData.arabicName}`, 'info');
    }
}
...
function centerAndScaleModel() {
    ...
    headModel.position.x = -center.x;
    headModel.position.y = -center.y;
    headModel.position.z = -center.z;
...
function setupEventListeners() {
    ...
    // تفاعل مع أيقونات/قائمة أجزاء الرأس (يمين الشاشة)
    document.querySelectorAll('.body-part-item').forEach(item => {
        item.addEventListener('click', () => {
            const partName = item.getAttribute('data-part');
            // تحديث الواجهة (العربي/الإنجليزي + التاريخ) من سكربت الـ HTML
            if (typeof window.handlePartSelection === 'function') {
                window.handlePartSelection(partName);
            }
            // تمييز الجزء على الموديل مع اختفاء الهايلايت بعد ثواني
...
function createBackupHeadModel() {
    ...
    showNotification('تم تحميل تفاصيل الرأس الاحتياطية', 'info');
}

// ===== INITIALIZE =====
window.addEventListener('DOMContentLoaded', () => {
    console.log("🧠 بدء تشغيل تفاصيل الرأس...");
