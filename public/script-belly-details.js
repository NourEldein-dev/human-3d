// ===== GLOBAL VARIABLES =====
let scene, camera, renderer, bellyModel;
...
// لون هايلايت موحد لكل أجزاء البطن
...
// خريطة أجزاء البطن حسب الفهرس
const bellyPartsByIndex = [
    { name: 'epigastric', ar: 'فُم المعدة', en: 'EPIGASTRIC', color: '#10b981', icon: 'custom-icon stomach' },
    { name: 'abdomen_up', ar: 'البطن العلوية', en: 'ABDOMEN_UP', color: '#10b981', icon: 'custom-icon stomach' },
    { name: 'abdomen_down', ar: 'البطن السفلية', en: 'ABDOMEN_DOWN', color: '#10b981', icon: 'custom-icon stomach' }
];


// ===== INITIALIZATION =====
async function init() {
    console.log("🚀 بدء تحميل تفاصيل البطن...");
...
viewer.innerHTML = '';
    ...
    console.log("✅ تهيئة تفاصيل البطن اكتملت!");
}
...
async function loadBellyModel() {
    console.log("🫁 جاري تحميل نموذج البطن التفصيلي...");
    
    try {
        const loader = new THREE.GLTFLoader();
        loader.load('abdomen.gltf', (gltf) => {
            console.log("✅ تم تحميل نموذج البطن!");
            
            bellyModel = gltf.scene;
            clickableObjects = [];
            
            // معالجة نموذج البطن
            processBellyModel(bellyModel);
            ...
            console.log(`🫁 تم العثور على ${clickableObjects.length} جزء من البطن`);
            
            showNotification('تم تحميل تفاصيل البطن بنجاح', 'success');
            
        }, undefined, (error) => {
            console.error('❌ خطأ في تحميل نموذج البطن:', error);
            showNotification('لا يمكن تحميل تفاصيل البطن', 'error');
...
// ===== PROCESS HEAD MODEL =====
function processBellyModel(bellyModel) {
    let partIndex = 0;
    ...
    bellyModel.traverse((child) => {
        if (child.isMesh) {
            ...
            // التعرف على جزء البطن حسب الفهرس
            const partInfo = bellyPartsByIndex[partIndex] || 
                           { name: 'Belly', ar: 'البطن', en: 'Belly', color: '#ef4444', icon: 'fas fa-lungs' };
    ...
    console.log(`📊 تم معالجة ${clickableObjects.length} جزء من البطن`);
}
...
function onModelClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        console.log("🎯 تم النقر على جزء من البطن!");
        ...
        showNotification('الرجاء النقر مباشرة على جزء من البطن', 'info');
    }
}
...
function centerAndScaleModel() {
    if (!bellyModel) return;
    
    const box = new THREE.Box3().setFromObject(bellyModel);
    const size = box.getSize(new THREE.Vector3());
    
    const maxSize = Math.max(size.x, size.y, size.z);
    const scale = 500 / maxSize;
    
    bellyModel.scale.set(scale, scale, scale);
    
    box.setFromObject(bellyModel);
    const center = box.getCenter(new THREE.Vector3());
    bellyModel.position.x = -center.x;
    bellyModel.position.y = -center.y;
    bellyModel.position.z = -center.z;
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.body-part-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            // إيجاد الميش المطابق لجزء البطن
...
function createBackupBellyModel() {
    ...
    showNotification('تم تحميل تفاصيل البطن الاحتياطية', 'info');
}

// ===== INITIALIZE =====
window.addEventListener('DOMContentLoaded', () => {
    console.log("🫁 بدء تشغيل تفاصيل البطن...");
