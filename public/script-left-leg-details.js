...
// لون هايلايت موحد لكل أجزاء الرجل اليسرى
...
// خريطة أجزاء الرجل اليسرى (حسب الاسم أو الفهرس)
const partMapByName = {
    'thigh_L': { name: 'thigh_L', ar: 'فخذ', en: 'Thigh', icon: 'custom-icon bicep_alt' },
    'knee_L': { name: 'knee_L', ar: 'ركبة', en: 'Knee', icon: 'custom-icon knee' },
    'shin_L': { name: 'shin_L', ar: 'ساق', en: 'Shin', icon: 'custom-icon shin' },
    'ankle_L': { name: 'ankle_L', ar: 'كاحل', en: 'Ankle', icon: 'custom-icon ankle' },
    'foot_L': { name: 'foot_L', ar: 'قدم', en: 'Foot', icon: 'custom-icon foot_sole' },
};

const partsByIndex = [
    { name: 'thigh_L', ar: 'فخذ', en: 'Thigh', icon: 'custom-icon bicep_alt' },
    { name: 'knee_L', ar: 'ركبة', en: 'Knee', icon: 'custom-icon knee' },
    { name: 'shin_L', ar: 'ساق', en: 'Shin', icon: 'custom-icon shin' },
    { name: 'ankle_L', ar: 'كاحل', en: 'Ankle', icon: 'custom-icon ankle' },
    { name: 'foot_L', ar: 'قدم', en: 'Foot', icon: 'custom-icon foot_sole' },
];


// ===== INITIALIZATION =====
async function init() {
    console.log("🚀 بدء تحميل تفاصيل الرجل اليسرى...");
...
viewer.innerHTML = '';
    ...
    console.log("✅ تهيئة تفاصيل الرجل اليسرى اكتملت!");
}
...
async function loadBellyModel() {
    console.log("🫁 جاري تحميل نموذج الرجل اليسرى التفصيلي...");
    
    try {
        const loader = new THREE.GLTFLoader();
        loader.load('leg_L.gltf', (gltf) => {
            console.log("✅ تم تحميل نموذج الرجل اليسرى!");
            
            armModel = gltf.scene;
            clickableObjects = [];
            
            // معالجة نموذج الرجل اليسرى
            ...
            console.log(`🫁 تم العثور على ${clickableObjects.length} جزء من الرجل اليسرى`);
            
            showNotification('تم تحميل تفاصيل الرجل اليسرى بنجاح', 'success');
            
        }, undefined, (error) => {
            console.error('❌ خطأ في تحميل نموذج الرجل اليسرى:', error);
            showNotification('لا يمكن تحميل تفاصيل الرجل اليسرى', 'error');
...
function processBellyModel(armModel) {
    ...
    console.log(`📊 تم معالجة ${clickableObjects.length} جزء من الرجل اليسرى`);
}
...
function onModelClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        console.log("🎯 تم النقر على جزء من الرجل اليسرى!");
        ...
        showNotification('الرجاء النقر مباشرة على جزء من الرجل اليسرى', 'info');
    }
}
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.body-part-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            // إيجاد الميش المطابق لجزء الرجل اليسرى
...
function createBackupBellyModel() {
    ...
    showNotification('تم تحميل تفاصيل الرجل اليسرى الاحتياطية', 'info');
}

// ===== INITIALIZE =====
window.addEventListener('DOMContentLoaded', () => {
    console.log("🫁 بدء تشغيل تفاصيل الرجل اليسرى...");
...
function translateSelected_leg_LTranslations(meshName) {
  return leg_LTranslations[meshName] || meshName;
}
