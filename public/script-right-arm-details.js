...
// لون هايلايت موحد لكل أجزاء الذراع الأيمن
...
// خريطة أجزاء الذراع الأيمن حسب الفهرس
const armPartsByIndex = [
    { name: 'shoulder',   ar: 'كتف',         en: 'Shoulder',   icon: 'custom-icon shoulder' },
    { name: 'upper_arm',  ar: 'يد علوية',   en: 'Upper Arm',  icon: 'custom-icon bicep' }, // bicep works nicely
    { name: 'elbow',      ar: 'كوع',         en: 'Elbow',      icon: 'fas fa-angles-right' },
    { name: 'forearm',    ar: 'يد سفلية',   en: 'Forearm',    icon: 'custom-icon forearm' },
    { name: 'wrist',      ar: 'معصم',        en: 'Wrist',      icon: 'fas fa-clock' },
    { name: 'hand',       ar: 'يد',          en: 'Hand',       icon: 'custom-icon hand' }
];


// ===== INITIALIZATION =====
async function init() {
    console.log("🚀 بدء تحميل تفاصيل الذراع الأيمن...");
...
viewer.innerHTML = '';
    ...
    console.log("✅ تهيئة تفاصيل الذراع الأيمن اكتملت!");
}
...
async function loadBellyModel() {
    console.log("🫁 جاري تحميل نموذج الذراع الأيمن التفصيلي...");
    
    try {
        const loader = new THREE.GLTFLoader();
        loader.load('arm_R.gltf', (gltf) => {
            console.log("✅ تم تحميل نموذج الذراع الأيمن!");
            
            armModel = gltf.scene;
            clickableObjects = [];
            
            // معالجة نموذج الذراع الأيمن
            ...
            console.log(`🫁 تم العثور على ${clickableObjects.length} جزء من الذراع الأيمن`);
            
            showNotification('تم تحميل تفاصيل الذراع الأيمن بنجاح', 'success');
            
        }, undefined, (error) => {
            console.error('❌ خطأ في تحميل نموذج الذراع الأيمن:', error);
            showNotification('لا يمكن تحميل تفاصيل الذراع الأيمن', 'error');
...
function processBellyModel(armModel) {
    ...
    armModel.traverse((child) => {
        if (child.isMesh) {
            ...
            // التعرف على جزء الذراع الأيمن حسب الفهرس
            const partInfo = armPartsByIndex[partIndex] || { name: 'hand', ar: 'يد', en: 'Hand', icon: 'fas fa-hand' };
    ...
    console.log(`📊 تم معالجة ${clickableObjects.length} جزء من الذراع الأيمن`);
}
...
function onModelClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        console.log("🎯 تم النقر على جزء من الذراع الأيمن!");
        ...
        showNotification('الرجاء النقر مباشرة على جزء من الذراع الأيمن', 'info');
    }
}
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.body-part-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            // إيجاد الميش المطابق لجزء الذراع الأيمن
...
function createBackupBellyModel() {
    ...
    showNotification('تم تحميل تفاصيل الذراع الأيمن الاحتياطية', 'info');
}

// ===== INITIALIZE =====
window.addEventListener('DOMContentLoaded', () => {
    console.log("🫁 بدء تشغيل تفاصيل الذراع الأيمن...");
